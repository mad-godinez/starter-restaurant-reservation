...
// eslint-disable-next-line
import {Route, useHistory,useRouteMatch, Redirect, Switch} from "react-router-dom";
...
export default function NewReservation(){
  ...
  // eslint-disable-next-line
  const history = useHistory(), {path} = useRouteMatch();
  const submitReservation = async (event) => {
    event.preventDefault();
    await upload(formData)
      .then(setResDate(
      // <Route path={`/dashboard?date=${formData.reservation_date}`}>
        <Routes date={formData.reservation_date}/>
      // </Route>
      ));
